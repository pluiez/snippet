...
|---|---|
| Git 分支 | `main` |
| 最新 commit | `b6dbab9 feat: error handling, dirty guards, startup warnings (Phase 3 Workflow B)` |
...
- **SPEC §13 全部核心不变量单元测试**（12 条，当前全部靠代码保证无 unit test）
- 关键 IPC 命令族的集成测试
- pinyin 索引的多音字回归测试
- fuzzy 匹配评分排序的回归测试
- Windows 安装包打包（.msi）
...
项目核心功能完整，动画打磨层和错误处理层已落地并验证。剩余 Phase 3 工作流 C（测试与发布）。
